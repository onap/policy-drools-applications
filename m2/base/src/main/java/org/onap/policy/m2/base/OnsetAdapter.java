...
public class OnsetAdapter implements Serializable {
    ...
    private static ConcurrentHashMap<Class,OnsetAdapter> map =
    ...
    public static OnsetAdapter get(ControlLoopEvent event) {
        ...
        if (rval == null) {
            ...
            if (matches.size() != 1) {
                // not the ideal situation -- either no matches, or multiple
                if (matches.size() == 0) {
                    System.out.println("ERROR: no matches for " + clazz);
                } else {
                    System.out.println("WARNING: multiple matches for " + clazz
                                       + matches + ": chose " + chosenMatch);
                }
            }
        }
        return rval;
