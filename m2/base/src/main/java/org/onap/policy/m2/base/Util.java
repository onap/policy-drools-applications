...
public class Util {
     ...
     * PolicyController.
     *
     * @param policyController the PolicyController instance associated with
     *     the sending Drools session
     ...
     */
    public static boolean deliver(PolicyController policyController, String topic, Object event) {
        // determine the topic type (UEB, DMAAP)
        List<? extends TopicSink> topicSinks =
            TopicEndpointManager.getManager().getTopicSinks(topic);
        if (topicSinks == null || topicSinks.size() != 1) {
            throw new IllegalStateException(
                "Cannot ensure correct delivery on topic "
             + topic + ": " + topicSinks);
        }
        return policyController.deliver(
                   topicSinks.get(0).getTopicCommInfrastructure(), topic, event);
    }
}
