...
public class Transaction implements Serializable {
    ...
    private static ConcurrentHashMap<String,Actor> nameToActor =
    ...
    public boolean isControlLoopEventValid(ControlLoopEvent onset) {
        if (onset.getRequestId() == null) {
            ...
            return false;
        } else if (onset.getTarget() == null || onset.getTarget().length() < 1) {
    ...
    public ControlLoopNotification getNotification(ControlLoopEvent event) {
        ...
        notification.setHistory(new LinkedList<ControlLoopOperation>(history));
    ...
    public ControlLoopNotification initialOperationNotification() {
        ...
        if (currentOperation != null && histEntry != null) {
            notification = onsetAdapter.createNotification(onset);
    ...
    private ControlLoopNotification processResult(PolicyResult result) {
        ...
        if (result != null) {
            ...
            if (saveHistEntry != null) {
                ...
                notification.setMessage(saveHistEntry.toHistory());

                // include the subset of history that pertains to this
                // actor and operation
                notification.setHistory(new LinkedList<ControlLoopOperation>());
                for (ControlLoopOperation clo : history) {
                    if (saveHistEntry.getOperation().equals(clo.getOperation())
                            && saveHistEntry.getActor().equals(clo.getActor())) {
                        notification.getHistory().add(clo);
                    }
                }
            }
                ...
                case FAILURE:
                    ...
                    if (attempt <= currentPolicy.getRetry()) {
                        ...
                        if (actor != null) {
                            ...
                            currentOperation =
                                actor.createOperation(this, currentPolicy, onset,
                                                      attempt);
                            createHistEntry();
                        } else {
     ...
     * instance.
     */
    public static interface Adjunct {
        /**
    ...
    private HashMap<Class, Adjunct> adjuncts = new HashMap<>();
    ...
    public boolean putAdjunct(Adjunct adjunct) {
        synchronized (adjuncts) {
            Class clazz = adjunct.getClass();
            if (adjuncts.get(clazz) != null) {
                // an adjunct with this class already exists
                return false;
            }
