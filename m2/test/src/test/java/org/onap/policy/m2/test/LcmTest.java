...
public class LcmTest {
    ...
    public static void init() throws Exception {
        ...
        closedLoopControlName = "lcm-" + UUID.randomUUID().toString();

        File kmodule = new File("src/test/resources/lcm/kmodule.xml");

        String pom = Util.openAndReplace("src/test/resources/lcm/pom.xml",
            "${project.version}", projectVersion);

        String yaml = Util.fileToString(new File("src/test/resources/lcm/CLRulevUSPLCMGuardTemplate.yaml"));

        // build a '.drl' file (as a String), by replacing '${variable}' names
        String drl = Util.openAndReplace(
            "src/test/resources/lcm/M2CLRulevUSPLCMGuardTemplate.drl",
            "${closedLoopControlName}", closedLoopControlName,
            "${controlLoopYaml}", Util.convertYaml(yaml),
            "${notificationTopic}", "NOTIFICATION-LCM-TOPIC",
            "${operationTopic}", "APPC-REQUEST-LCM-TOPIC",
            "${policyName}", "lcm",
            "${policyScope}", "service=vUSP;resource=vCTS;type=operational" ,
            "${policyVersion}",
            "org.onap.policy.m2.test:lcm:" + projectVersion,
            "${unique}", "2");
        ...
        properties = PropertyUtil.getProperties("src/test/resources/lcm/controller.properties");
        properties.setProperty("rules.version", projectVersion);

        // create PolicyController, which creates the Drools session
        policyController =
            PolicyEngineConstants.getManager().createPolicyController("lcm", properties);
        policyController.start();

        // create writers
        dcae = new Output("org.onap.DCAE-LCM-TOPIC");
        appcResponse = new Output("APPC-RESPONSE-LCM-TOPIC");

        // create readers
        notification = new Input("NOTIFICATION-LCM-TOPIC");
        appcRequest = new Input("APPC-REQUEST-LCM-TOPIC");
    }
    ...
    public void sunnyDayTest() throws Exception {
        ...
        // send ABATED
        req.msg.put("closedLoopEventStatus", "ABATED");
        dcae.send(req.msg);
            ...
            json("correlation-id", opRequest.get("correlation-id"),
                 "body", json("output",
                              json("common-header",
                                   json("flags", json(),
                                        "api-ver", "2.00",
                                        "originator-id", ch.opt("originator-id"),
                                        "sub-request-id", ch.opt("sub-request-id"),
                                        "request-id", req.requestId,
                                        "timestamp", ch.opt("timestamp")
                                       ),
                                   "status",
