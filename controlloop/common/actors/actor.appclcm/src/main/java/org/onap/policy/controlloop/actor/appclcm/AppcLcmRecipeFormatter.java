...
class AppcLcmRecipeFormatter {
    ...
    private final List<String> splitedRecipe;

    AppcLcmRecipeFormatter(String rawRecipe) {
        splitedRecipe = Lists.newArrayList(rawRecipe.split(SPLIT_SIGN));
    }

    String getURLRecipe() {
        return StringUtils.join(lowercaseStrings(splitedRecipe), SPLIT_SIGN);
    }

    String getBodyRecipe() {
        return StringUtils.join(capitalizeStrings(splitedRecipe), BODY_SEPARATOR);
    }
