...
public class SDNRActorServiceProvider implements Actor {
    ...
    private static final ImmutableList<String> recipes =
            ImmutableList.of(RECIPE_MODIFY);
    ...
    /**
     * Constructs an SDNR request conforming to the pci API. The actual request is constructed and
    ...
    public static PciRequestWrapper constructRequest(VirtualControlLoopEvent onset
    		, ControlLoopOperation operation, Policy policy) {

        /* Construct an SDNR request using pci Model */

        /*
         * The actual pci request is placed in a wrapper used to send through dmaap. The current
        ...
        /*
         * An action is required for all SDNR requests, this will be the recipe specified in the
         * policy.
         */
        sdnrRequest.setAction(
                policy.getRecipe());
     ...
     * 
     * @param subRequestId the sub id used to send to SDNR, Policy sets this using the operation
     *        attempt
     * 
    ...
    /**
     * Processes the SDNR pci response sent from SDNR. Determines if the SDNR operation was
     * successful/unsuccessful and maps this to the corresponding Policy result.
     * 
     * @param dmaapResponse the dmaap wrapper message that contains the actual SDNR reponse inside
     *        the body field
     * 
     * @return an key-value pair that contains the Policy result and SDNR response message
     */
    public static SimpleEntry<PolicyResult, String> processResponse(PciResponseWrapper dmaapResponse) {
        ...
        logger.info("SDNR Response Message is {}", responseValue);
        
        /* Response and Payload are just printed and no further action needed in casablanca release */  
