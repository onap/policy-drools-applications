...
public class TdjamController extends NonDroolsPolicyController {
    ...
    private void processEvent(ControlLoopEvent event) {
        ...
        if (event instanceof CanonicalOnset) {
            ...
            if (manager != null) {
                if (manager.getSerialWorkQueue().isRunning()) {
                    // existing manager - this event manager should process it as a
                    // subsequent event, even if the requestId is different
                    manager.subsequentEvent(coEvent);
        ...
        } else if (event instanceof VirtualControlLoopEvent) {
