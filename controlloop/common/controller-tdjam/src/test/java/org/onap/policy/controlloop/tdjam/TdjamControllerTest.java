...
public class TdjamControllerTest {
    ...
    public void onsetErrors() throws Exception {
        ...
        tc.start();
        final Listener<VirtualControlLoopNotification> policyClMgt =
            topics.createListener("POLICY-CL-MGT", VirtualControlLoopNotification.class, tc);
        ...
        // this should handle a 'null' (missing) requestId
        canonicalOnset.setClosedLoopControlName("clpa1");
        Whitebox.invokeMethod(tc, "processEvent", canonicalOnset);
        VirtualControlLoopNotification notification = policyClMgt.await(30L, TimeUnit.SECONDS);
        assertEquals("clpa1", notification.getClosedLoopControlName());
        assertEquals("REJECTED", notification.getNotification().toString());
        assertEquals("Missing requestId", notification.getMessage());

        // try with a non-null requestID, but missing target
        canonicalOnset.setRequestId(UUID.randomUUID());
        startLog();
        Whitebox.invokeMethod(tc, "processEvent", canonicalOnset);
        stopLog();
