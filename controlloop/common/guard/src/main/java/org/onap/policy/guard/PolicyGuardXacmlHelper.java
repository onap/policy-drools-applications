...
/**
 * Based off of GuardContext.java. Much code is reused here
 * verbatim. Some functionality is not necessary but not removed
 *  to keep close similarity.
 * 
 */
public class PolicyGuardXacmlHelper {
	...
	public String callPDP(String restfulPdpUrl,
		...
		try {
			...
			response = callRESTfulPDP(new ByteArrayInputStream(jsonReq
					.toString().getBytes()),
					new URL(restfulPdpUrl/* "https://localhost:8443/pdp/" */), urlEntry.authorization,
	 ...
	 */
	private static String callRESTfulPDP(InputStream is, URL restURL, String authorization,
	...
	private void init(Properties properties) {
		...
		String defaultPassword = properties
				.getProperty("guard.pdp.rest.password");
		String defaultClientUser = properties
				.getProperty("guard.pdp.rest.client.user");
		String defaultClientPassword = properties
				.getProperty("guard.pdp.rest.client.password");
		String defaultEnvironment = properties
				.getProperty("guard.pdp.rest.environment");
		...
		for (int index = 0; index < 10; index += 1) {
			...
			for (String restURL : restURLlist.split("\\s*;\\s*")) {
				...
				String clientUser = properties.getProperty(prefix
						+ "client.user", defaultClientUser);
				String clientPassword = properties.getProperty(prefix
				...
				String environment = properties.getProperty(prefix
		...
		for (String propertyName : new String[] { "driver", "url", "user",
				"password" }) {
			...
			if (value != null) {
				...
				sb.append("\"'").append(guardPropertyName)
