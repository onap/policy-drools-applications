...
public class UsecasesEventManager extends ControlLoopEventManager {
    ...
    private synchronized void startOperation() throws ControlLoopException {
        ...
        if ((finalResult = getProcessor().checkIsCurrentPolicyFinal()) == null) {
            ...
            currentOperation.set(makeOperationManager(context, getProcessor().getCurrentPolicy()));
            currentOperation.get().start(getEndTimeMs() - System.currentTimeMillis());
            return;
        }
    ...
    public synchronized void nextStep() {
        ...
        try {
            if (!currentOperation.get().nextStep()) {
                ...
                getProcessor().nextPolicyForResult(currentOperation.get().getOperationResult());
                startOperation();
            }

        } catch (ControlLoopException | RuntimeException e) {
            // processor problem - this is fatal
            logger.warn("{}: cannot start next step for {}", closedLoopControlName, getRequestId(), e);
    ...
    public synchronized void updated(ControlLoopOperationManager2 operation) {
        ...
        switch (operation.getState()) {
            ...
            case CONTROL_LOOP_TIMEOUT:
                logger.warn("{}: control loop timed out for {}", closedLoopControlName, getRequestId());
                controlLoopHistory.addAll(currentOperation.get().getHistory());
                notification.setNotification(ControlLoopNotificationType.FINAL_FAILURE);
