...
public class UsecasesEventManager extends ControlLoopEventManager implements StepContext {
    ...
    /**
     * This is populated by GetTargetEntityOperation2.
     */
    private AtomicReference<String> targetEntity = new AtomicReference<>();
    ...
    /**
     * Gets the current value of the target entity.
     *
     ...
     */
    public void abort(@NonNull State finalState, FinalResult finalResult) {
    ...
    public void loadPreprocessorSteps() {
         ...
         */
        if (needPolicySteps) {
            steps.push(new LockStep2(step, targetEntity));
        }

        /*
         * Need to determine target entity first.
         */
        if (targetEntity.get() == null && !(step instanceof GetTargetEntityStep2)) {
            steps.push(new GetTargetEntityStep2(step, targetEntity));
        }
    }
