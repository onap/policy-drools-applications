...
/**
 * Steps specific to the usecases controller. It has a {@link #setProperties()} method
 * that knows how to load the various properties into the operation, extracting enrichment
 * data where appropriate, and extracting other data from the step's context. For each
 * property, there is a getXxx() method for extracting the value and a loadXxx() method
 * for loading the extracted value into the operation. In addition, it has a
 * {@link #success(OperationOutcome)} method that is responsible for extracting responses
 * from an operation outcome and recording the data in the step's context for use by
 * subsequent steps.
 */
public class Step2 extends Step {
    ...
    public List<String> getPropertyNames() {
        List<String> names = getOperation().getPropertyNames();

        if (names.contains(OperationProperties.AAI_PNF)) {
            // need the target entity to look up the PNF
            names = new ArrayList<>(names);
            names.add(OperationProperties.AAI_TARGET_ENTITY);
        }
