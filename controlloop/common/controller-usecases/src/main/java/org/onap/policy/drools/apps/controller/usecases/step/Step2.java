...
public class Step2 extends Step {
    ...
    public List<String> getPropertyNames() {
        return getOperator().getPropertyNames(params);
    }

    /**
     * Gets the operator associated with the step.
     *
     * @return the operator
     */
    protected Operator getOperator() {
        return params.getActorService().getActor(params.getActor()).getOperator(params.getOperation());
    }

    @Override
    protected Operation buildOperation() {
        return getOperator().buildOperation(params);
    }
    ...
    protected StandardCoderObject getPnf() {
        return stepContext.getProperty(AaiGetPnfOperation.getKey(params.getTargetEntity()));
    }
