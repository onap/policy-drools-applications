...
public final class SdncManager implements Runnable {
    ...
    public SdncManager(WorkingMemory wm, SdncRequest request) {
        ...
        setSdncParams(getPEManagerEnvProperty("sdnc.url"), "", "");
    }
    ...
    public void run() {
        ...
        try {
            ...
            httpDetails = restManager.post(sdncUrl, "", "", headers, "application/json",
                                           sdncRequestJson);
        } catch (Exception e) {
        ...
        try {
            ...
            if (!response.getResponseOutput().getResponseCode().equals("200")) {
                logger.info(
                    "Sdnc Heal Restcall failed with http error code a " + httpDetails.first + " " + httpDetails.second
                );
            }
