...
public class TestVFCManager {
	...
	//when(mockedRESTManager.post(startsWith("http://somewhere.over.the.rainbow"), eq("Dorothy"), eq("Toto"), anyMap(), anyString(), anyString())).thenReturn(httpResponseErr1);
	//when(mockedRESTManager.post(anyString(), anyString(), anyString(), anyMap(), anyString(), anyString())).thenReturn(httpResponseOK);
