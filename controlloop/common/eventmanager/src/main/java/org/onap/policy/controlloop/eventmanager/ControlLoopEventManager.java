...
public class ControlLoopEventManager implements Serializable {
    ...
    private static final String PNF_IS_CLOSED_LOOP_DISABLED = "pnf.is-closed-loop-disabled";
    ...
    static {
        VALID_TARGETS = Collections.unmodifiableSet(new HashSet<>(
                Stream.of(VM_NAME, VNF_NAME, VSERVER_VSERVER_NAME, GENERIC_VNF_VNF_ID, GENERIC_VNF_VNF_NAME, PNF_ID,
                        PNF_NAME).map(String::toLowerCase).collect(Collectors.toList())));
    }
    ...
    private void validateAaiData(VirtualControlLoopEvent event) throws ControlLoopException {
        ...
        switch (event.getTargetType()) {
            ...
            case VNF:
                ...
                return;
            case PNF:
                if (eventAai.get(PNF_ID) == null || eventAai.get(PNF_NAME) == null) {
                    throw new ControlLoopException("pnf-id or pnf-name information missing");
                }
                return;
            default:
    ...
    public void queryAai(VirtualControlLoopEvent event) throws AaiException {
        ...
        // TODO: Cleanup the AAI enrichment by target-type.
        ...
        if (aai.containsKey(VSERVER_IS_CLOSED_LOOP_DISABLED) || aai.containsKey(GENERIC_VNF_IS_CLOSED_LOOP_DISABLED)
                    || aai.containsKey(PNF_IS_CLOSED_LOOP_DISABLED)) {

            if (isClosedLoopDisabled(event)) {
                throw new AaiException("is-closed-loop-disabled is set to true on VServer or VNF or PNF");
            }
    ...
    public Map<String, String> getPnf(VirtualControlLoopEvent event) throws AaiException {
        ...
        if (!aai.containsKey(PNF_ID) || !aai.containsKey(PNF_NAME)) {
            throw new AaiException("Missing identifier for PNF in the event. Pass either the ID or name");
        }

        UUID reqId = event.getRequestId();
        String pnfId = event.getAai().get(PNF_NAME);
        String aaiHostUrl = PolicyEngineConstants.getManager().getEnvironmentProperty(AAI_URL);
        String aaiUser = PolicyEngineConstants.getManager().getEnvironmentProperty(AAI_USERNAME_PROPERTY);
        String aaiPassword = PolicyEngineConstants.getManager().getEnvironmentProperty(AAI_PASS_PROPERTY);

        Map<String, String> pnfParams =
                new AaiManager(new RestManager()).getPnf(aaiHostUrl, aaiUser, aaiPassword, reqId, pnfId);
