...
public abstract class ControlLoopEventManager implements StepContext, Serializable {
    ...
    /**
     * Outcomes resulting in an abort are added to this queue. This would include a lock
     * being lost or a timeout of the entire control loop.
     */
    @Getter
    private final transient Deque<OperationOutcome> abortOutcomes = new ConcurrentLinkedDeque<>();

    /**
     * Time when the current policy started executing.
     */
    @Getter
    ...
    public synchronized void destroy() {
        // TODO do this in the foreground thread?
    ...
    public synchronized CompletableFuture<OperationOutcome> requestLock(String targetEntity) {
        ...
        data.addUnavailableCallback(this::abort);

        CompletableFuture<OperationOutcome> future = data.getFuture();
        future.whenComplete((outcome, thrown) -> onComplete(outcome));

        return future;
    }

    protected void clearStartTime() {
        startTime = null;
    }
