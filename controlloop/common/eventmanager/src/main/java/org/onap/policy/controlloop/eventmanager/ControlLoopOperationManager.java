...
public class ControlLoopOperationManager implements Serializable {
	...
	private void	storeOperationInDataBase(){
		...
		try {
			//props = PropertyUtil.getProperties("src/main/resources/operation_history.properties"); // Does not work
			//props.load(new FileInputStream(new File("src/main/resources/operation_history.properties"))); // Does not work
			//InputStream is = new FileInputStream(new File("../guard/src/main/resources/operation_history.properties")); // Does not work
			InputStream is = org.onap.policy.guard.PIPEngineGetHistory.class.getResourceAsStream("/operation_history.properties"); // Works but creates error elsewhere
			props.load(is);
		} catch (Exception ex) {
