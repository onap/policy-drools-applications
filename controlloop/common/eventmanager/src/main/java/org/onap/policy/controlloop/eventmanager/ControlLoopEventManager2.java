...
public class ControlLoopEventManager2 implements ManagerContext, Serializable {
    ...
    private synchronized void startOperation() throws ControlLoopException {
        ...
        logger.info("final={} oper state={}", finalResult, currentOperation.get().getState());
    ...
    public VirtualControlLoopNotification makeNotification() {
        ...
        if (finalResult == null) {
            ControlLoopOperationManager2 op = currentOperation.get();
            if (op != null) {
                notif.setMessage(op.getOperationMessage());
                notif.setHistory(op.getHistory());
            }
        }
