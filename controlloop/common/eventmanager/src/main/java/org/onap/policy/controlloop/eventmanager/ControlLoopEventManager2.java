...
public class ControlLoopEventManager2 implements ManagerContext, Serializable {
    ...
    private static final long DEFAULT_TIMEOUT_MS = TimeUnit.MILLISECONDS.convert(90, TimeUnit.MINUTES);
    ...
    public synchronized CompletableFuture<OperationOutcome> requestLock(String targetEntity,
        ...
        LockData data = target2lock.computeIfAbsent(targetEntity, key -> new LockData(targetEntity, requestId));
        data.addUnavailableCallback(lockUnavailableCallback);

        int remainingSec = Math.max(5,
                        (int) TimeUnit.SECONDS.convert(endTimeMs - System.currentTimeMillis(), TimeUnit.MILLISECONDS));

        if (useTargetLock) {
            createRealLock(targetEntity, requestId, remainingSec, data);
        } else {
            createPseudoLock(targetEntity, requestId, remainingSec, data);
        }
    ...
    private static class LazyInitData {
        ...
        static {
            ...
            OperationHistoryDataManager dataManager;
            try {
                GuardOperator guard = (GuardOperator) ACTOR_SERVICE.getActor(GuardActorServiceProvider.NAME)
                                .getOperator(GuardOperation.NAME);
                GuardConfig config = (GuardConfig) guard.getCurrentConfig();

                dataManager = (config.isDisabled() ? new OperationHistoryDataManagerStub()
                                : new OperationHistoryDataManagerImpl());

            } catch (RuntimeException e) {
                logger.warn("cannot check 'disabled' property in GUARD actor - assuming enabled", e);
                dataManager = new OperationHistoryDataManagerImpl();
            }
