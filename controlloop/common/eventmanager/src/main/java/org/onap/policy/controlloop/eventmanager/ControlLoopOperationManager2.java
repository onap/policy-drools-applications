...
public class ControlLoopOperationManager2 implements Serializable {
    ...
    /**
     * Handles exceptions, including control loop timeout, that may be generated.
     *
    ...
    private CompletableFuture<OperationOutcome> requestLock() {
        ...
        lockFuture.thenAccept(outcome -> {
            /*
             * Note: only pass FAILUREs to onComplete().
             */
    ...
    private void lockUnavailable(OperationOutcome outcome) {
        logger.warn("{}.{}: lock was lost for {}", actor, operation, requestId);
        ...
        /*
         * Now that we've added the lock failure to the queue, ensure the future is
    ...
    /**
     * Handles responses provided via the "complete" callback. Note: this is never be
     * invoked for "successful" locks.
     *
     * @param outcome outcome provided to the callback
     */
