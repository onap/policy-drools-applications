...
public class ControlLoopOperationManager2 implements Serializable {
    ...
    public ControlLoopOperationManager2(ManagerContext operContext, ControlLoopEventContext context, Policy policy) {
        ...
        // TODO use DroolsExecutor in ControlLoopOperationParams
    ...
    /**
     * Handles exceptions generated by the operation.
     *
     * @param thrown exception that was generated
     */
    private synchronized void handleException(Throwable thrown) {
    ...
    private CompletableFuture<OperationOutcome> detmVfModuleTarget() {
        String targetType2 = eventContext.getEvent().getTarget();
        if (targetType2 == null) {
    ...
    private CompletableFuture<OperationOutcome> requestLock() {
        CompletableFuture<OperationOutcome> lockFuture = operContext.requestLock(targetEntity, this::lockUnavailable);
    ...
    private void storeOperationInDataBase() {
        operContext.getDataManager().store(eventContext.getEvent(), operationHistory.peekLast().getClOperation());
    }

    /**
     * Context for the Operation Manager.
     */
    public static interface ManagerContext {
         ...
         * @return a future to await the lock
         */
        CompletableFuture<OperationOutcome> requestLock(String target, Runnable lockUnavailableCallback);
