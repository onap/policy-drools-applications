...
public class ControlLoopOperationManager2 implements Serializable {
    ...
    /**
     * Start the operation, acquiring any locks that are needed first.
     */
    ...
    private CompletableFuture<OperationOutcome> requestLock() {
        CompletableFuture<OperationOutcome> lockFuture = lockManager.apply(targetEntity, this::lockLost);
    ...
    /**
     * Indicates that the lock on the target entity has been lost.
     */
    private synchronized void lockLost() {
        if (future != null) {
            future.cancel(false);
        }
        ...
        outcome.setMessage(LOCK_LOST_MSG);
        onComplete(outcome);
    }
    ...
    private void processOutcome() {
        ...
        switch (outcome.getActor()) {
            ...
            case LOCK_ACTOR:
                if (LOCK_LOST_MSG.equals(outcome.getMessage())) {
